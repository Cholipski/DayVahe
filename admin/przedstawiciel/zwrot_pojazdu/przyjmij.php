...
$stmt_k = $db->prepare("SELECT wynajem.id, wynajem.data_rozpoczecia, wynajem.data_zakonczenia, klient.imie, klient.nazwisko, klient.pesel, pojazd.marka, pojazd.model, pojazd.numer_rejestracyjny, pojazd.kaucja FROM wynajem LEFT JOIN klient ON klient.id = wynajem.klient LEFT JOIN pojazd ON pojazd.id = wynajem.pojazd WHERE wynajem.status = 'wynajety' ORDER BY wynajem.id");
$stmt_k->execute();
...
<head>
    ...
    <title>Day Vahe - Zwróć wynajem</title>
...
<div class="container-fluid m-0 p-0 bg-form" style="height: 100vh">
    <div class="add-car-form-container p-4">
        <div class="card w-50 mt-5">
            <div class="card-body">
                <h3 class="card-title text-center font-weight-bold">Zwróć wynajem</h3>
                <div class="card-text">
                        ...
                        <div class="form-group">
                            ...
                            <select name="wynajem" id="fwynajem" class="selectpicker form-control form-control-sm" data-live-search="true">
                                ...
                                while($row = $stmt_k->fetch(PDO::FETCH_ASSOC)){
                                    ...
                                    value='.$row['id'].'>'.$row['id'].' '.$row['data_rozpoczecia'].' '.$row['data_zakonczenia'].' '.$row['imie'].' '.$row['nazwisko'].' '.$row['pesel'].' '.$row['marka'].' '.$row['model'].' '.$row['numer_rejestracyjny'].'</option>';
                                }
                                ?>
                            </select>
                        </div>

                        <button type="submit" class="btn btn-primary btn-block">Zwróć</button>
                        <div class="sign-up">
