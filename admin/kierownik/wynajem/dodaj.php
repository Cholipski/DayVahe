...
$stmt_k = $db->prepare("SELECT * FROM `klient`");
$stmt_k->execute();
...
<div class="container-fluid m-0 p-0 bg-form" style="height: 100vh">
    <div class="add-car-form-container p-4">
        <div class="card w-50 mt-5">
            <div class="card-body">
                <h3 class="card-title text-center font-weight-bold">Dodaj nowy wynajem</h3>
                <div class="card-text">
                    <form action="wynajemC.php" method="post">
                        <div class="form-group">
                            <label for="fklient">Wybierz klienta: </label>
                            <select name="klient" id="fklient" class="selectpicker form-control form-control-sm" data-live-search="true">
                                <option value="" disabled selected class="pl-3">Wybierz klienta</option>
                                <?php
                                while($row = $stmt_k->fetch(PDO::FETCH_ASSOC)){
                                    echo '<option class="pl-3" data-tokens="'.$row['imie'].' '.$row['nazwisko'].' '.$row['pesel'].'"
                                value='.$row['id'].'>'.$row['id'].' '.$row['imie'].' '.$row['nazwisko'].' '.$row['pesel'].'</option>';
                                }
                                ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="fstatus" class="form-control form-control-sm text-muted" placeholder="Wybierz status">
                                <option value="zarezerwowany" class="pl-3">zarezerwowany</option>
                                <option value="wynajety" class="pl-3">wynajety</option>
                                <option value="zwrocony" class="pl-3">zwrocony</option>
                            </select>
                        </div>


                        <button type="submit" class="btn btn-primary btn-block">Dodaj wynajem</button>
                        <div class="sign-up">
